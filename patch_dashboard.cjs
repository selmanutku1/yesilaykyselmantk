...
// Ensure Sparkles and Mic are imported
if (!content.includes('Sparkles')) {
  content = content.replace("} from 'lucide-react';", "  Sparkles,\n  Mic,\n} from 'lucide-react';");
} else if (!content.includes('Mic')) {
  content = content.replace("} from 'lucide-react';", "  Mic,\n} from 'lucide-react';");
}

const targetStr = `      {/* Günlük Kamp Programı & Google Calendar Takvimi */}
      <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm print:hidden">`;

const updatesHtml = `      {/* Sistem Güncellemeleri - Beta */}
...
content = content.replace(targetStr, updatesHtml);
