...
const originalJSX = `              {/* CONSENTS & CONTRACTS FOR GROUP */}
...
const newJSX = `              {/* CONSENTS & CONTRACTS FOR GROUP */}
...
if (content.includes("KVKK Onayı:</strong> Sorumlusu olduğum")) {
  content = content.replace(originalJSX, newJSX);
  fs.writeFileSync('src/components/RegistrationView.tsx', content);
  console.log("Replaced convoy consents");
} else {
  console.log("Could not find the exact original text. Printing what is around 2540-2575:");
  console.log(content.split('\n').slice(2530, 2580).join('\n'));
}
