...
export default function SurveyAnalysisView({ 
  ...
  const surveyData = surveys.map(s => ({
    id: s.id,
    name: participants.find(p => p.id === s.participantId)?.name || 'Bilinmiyor',
    camp: 'Belirtilmemiş',
    period: periods.find(p => p.id === s.campPeriodId)?.name || 'Belirtilmemiş',
    date: 'Tarih belirtilmemiş',
    genel: ((s.ratingMeals + s.ratingActivities + s.ratingBungalows + s.ratingTrainers) / 4),
    tesis: s.ratingBungalows,
    egitim: s.ratingTrainers,
    etkinlik: s.ratingActivities,
    yemek: s.ratingMeals,
    feedback: s.generalComment
  }));
  const mockSurveyData = surveyData.length > 0 ? surveyData : [
  {
    id: 1, name: 'Örnek Katılımcı', camp: 'Sistem Verisi Yok', period: 'Dönem Yok', date: 'Tarih Yok',
    genel: 0, tesis: 0, egitim: 0, etkinlik: 0, temizlik: 0, yemek: 0, guvenlik: 0, liderler: 0,
    feedback: 'Henüz anket verisi bulunmamaktadır.'
  }
];
